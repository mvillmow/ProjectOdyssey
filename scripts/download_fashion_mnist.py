...
Downloads and extracts Fashion-MNIST dataset in IDX format.

Usage:
    python scripts/download_fashion_mnist.py [output_dir]

Default output: datasets/fashion_mnist
"""
...
def download_fashion_mnist(output_dir: str):
    ...
    # Fashion-MNIST file mappings: (source_filename, output_filename)
    file_mappings = [
    ...
    for gz_filename, output_filename in file_mappings:
        ...
        if not output_file_path.exists():
            url = f"{FASHION_MNIST_BASE_URL}/{gz_filename}"
            print(f"Downloading Fashion-MNIST {output_filename.split('_')[0]} data...")
            print(f"  From: {url}")
    ...
    print(f"\n✓ Fashion-MNIST dataset ready at: {output_path}")


if __name__ == "__main__":
    output_dir = sys.argv[1] if len(sys.argv) > 1 else "datasets/fashion_mnist"
    download_fashion_mnist(output_dir)
