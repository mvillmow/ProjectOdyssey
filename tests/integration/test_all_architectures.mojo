...
fn test_model_forward(
    ...
    try:
        ...
        print(f"  Output shape: ({batch_out}, {classes_out})")

        if batch_out != batch_size:
            print(f"✗ FAIL: Expected batch size {batch_size}, got {batch_out}")
            return False

        if classes_out != 10:
            print(f"✗ FAIL: Expected 10 classes, got {classes_out}")
            return False
        ...
        print(f"  Output shape: ({batch_train}, {classes_train})")

        if batch_train != batch_size:
            print(f"✗ FAIL: Expected batch size {batch_size}, got {batch_train}")
            return False

        if classes_train != 10:
            print(f"✗ FAIL: Expected 10 classes, got {classes_train}")
            return False
        ...
        print(f"  {same_pct:.1f}% of outputs are identical")
