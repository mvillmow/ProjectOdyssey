...
fn example_basic_signed_integers() raises:
    ...
    print("Int8 arithmetic: 42+10=", str(sum), " 42-5=", str(diff), " 42*2=", str(prod))
    ...
    print("Clamping: 200 ->", str(clamped_high), " -200 ->", str(clamped_low))
...
fn example_basic_unsigned_integers() raises:
    ...
    var u8_1 = UInt8(255)
    var u8_2 = UInt8(100)
    print("UInt8 values:", str(u8_1), str(u8_2))

    # Arithmetic operations
    var sum = u8_2 + UInt8(50)
    var diff = u8_1 - u8_2
    print("UInt8 arithmetic: 100+50=", str(sum), " 255-100=", str(diff))

    # Clamping demonstration
    var clamped_high = UInt8(300)  # Exceeds max (255)
    var clamped_low = UInt8(-50)   # Below min (0)
    print("Clamping: 300 ->", str(clamped_high), " -50 ->", str(clamped_low))
...
fn example_type_conversions() raises:
    ...
    var i32 = i8.to_int32()
    var i64 = i8.to_int64()
    print("Int8(42) -> Int16:", str(i16), " Int32:", str(i32), " Int64:", str(i64))

    # Larger types to Int8 (with clamping)
    var large_i32 = Int32(1000)
    var back_to_i8 = large_i32.to_int8()
    print("Int32(1000) -> Int8:", str(back_to_i8), " (clamped to 127)")

    # Float conversions
    var from_float = Int8.from_float32(42.7)
    var to_float = from_float.to_float32()
    print("Float32(42.7) -> Int8:", str(from_float), " -> Float32:", str(to_float))
...
fn example_tensor_conversions() raises:
    ...
    print(" ", str(i8_tensor._data.bitcast[Int8]()[0]),
          str(i8_tensor._data.bitcast[Int8]()[1]),
          str(i8_tensor._data.bitcast[Int8]()[2]),
          str(i8_tensor._data.bitcast[Int8]()[3]),
          str(i8_tensor._data.bitcast[Int8]()[4]),
          str(i8_tensor._data.bitcast[Int8]()[5]))

    # Convert to UInt8
    var u8_tensor = t.to_uint8()
    print("\nAfter conversion to UInt8 (truncated and clamped):")
    print(" ", str(u8_tensor._data.bitcast[UInt8]()[0]),
          str(u8_tensor._data.bitcast[UInt8]()[1]),
          str(u8_tensor._data.bitcast[UInt8]()[2]),
          str(u8_tensor._data.bitcast[UInt8]()[3]),
          str(u8_tensor._data.bitcast[UInt8]()[4]),
          str(u8_tensor._data.bitcast[UInt8]()[5]))
...
fn example_16bit_integers() raises:
    ...
    var i16_2 = Int16(-500)
    print("Int16 values:", str(i16_1), str(i16_2))

    # UInt16
    var u16_1 = UInt16(60000)
    var u16_2 = UInt16(5000)
    print("UInt16 values:", str(u16_1), str(u16_2))

    # Arithmetic
    var i16_sum = i16_1 + i16_2
    var u16_sum = u16_1 + u16_2
    print("Int16 sum:", str(i16_sum), " UInt16 sum:", str(u16_sum))

    # Clamping boundaries
    var i16_max = Int16(40000)  # Exceeds 32767
    var u16_max = UInt16(70000)  # Exceeds 65535
    print("Int16 clamping: 40000 ->", str(i16_max))
    print("UInt16 clamping: 70000 ->", str(u16_max))
...
fn example_32bit_integers() raises:
    ...
    print("Int32 value:", str(i32))

    # UInt32
    var u32 = UInt32(4000000000)
    print("UInt32 value:", str(u32))

    # Conversions
    var i32_from_i8 = Int32.from_int8(Int8(42).value)
    var u32_from_u16 = UInt32.from_uint16(UInt16(1000).value)
    print("Int32 from Int8(42):", str(i32_from_i8))
    print("UInt32 from UInt16(1000):", str(u32_from_u16))
...
fn example_64bit_integers() raises:
    ...
    print("Int64 value:", str(i64))

    # UInt64
    var u64 = UInt64(18446744073709551615)
    print("UInt64 large value:", str(u64))
    ...
    print("Int64 from Int32:", str(i64_from_i32))
    print("UInt64 from UInt32:", str(u64_from_u32))
...
fn example_practical_use_case() raises:
    ...
    print("Scenario: Quantizing a float32 tensor to UInt8 for memory efficiency")

    # Create a float tensor with normalized values (0.0 to 1.0)
    var normalized = zeros(List[Int](5,), DType.float32)
    ...
    var scaled = zeros(List[Int](5,), DType.float32)
    for i in range(5):
        scaled._data.bitcast[Float32]()[i] = normalized._data.bitcast[Float32]()[i] * 255.0
    ...
    print(" ", str(quantized._data.bitcast[UInt8]()[0]),
          str(quantized._data.bitcast[UInt8]()[1]),
          str(quantized._data.bitcast[UInt8]()[2]),
          str(quantized._data.bitcast[UInt8]()[3]),
          str(quantized._data.bitcast[UInt8]()[4]))

    # Dequantize back to float
    var dequantized = zeros(List[Int](5,), DType.float32)
    for i in range(5):
        dequantized._data.bitcast[Float32]()[i] = Float32(quantized._data.bitcast[UInt8]()[i]) / 255.0

    print("\nDequantized back to 0.0-1.0 range:")
    print(" ", str(dequantized._data.bitcast[Float32]()[0]),
          str(dequantized._data.bitcast[Float32]()[1]),
          str(dequantized._data.bitcast[Float32]()[2]),
          str(dequantized._data.bitcast[Float32]()[3]),
          str(dequantized._data.bitcast[Float32]()[4]))

    print("\nNote: Some precision is lost due to quantization (e.g., 0.25 -> 0.24901961)")
