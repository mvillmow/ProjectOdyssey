...
1. **Commented Code** (lines 160-181):
...
1. **Consolidate**: If they're identical, keep one version
1. **Differentiate**: Make them clearly distinct (different signatures, purposes)
...
1. **Keep TODO** if time module not ready (track in issue #1538)
1. **Remove** if not actually used in any tests
...
1. **Remove comments** if not needed soon
1. **Convert to docstring TODO** if implementation is planned
...
1. **Remove file** if not needed
1. **Keep as tracking TODO** if implementation is near-term
...
1. **Decide consolidation** strategy:
   - Remove duplicates
...
1. **Check import paths** still work
1. **Validate CI** still passes

### Phase 5: Final Documentation

1. **Update API reference** with any changes
1. **Update examples** if implementations changed
1. **Document cleanup** decisions
