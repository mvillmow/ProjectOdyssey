...
1. **Implemented RandomRotation** - Full rotation with nearest-neighbor sampling
1. **Added Pipeline Type Alias** - Backward-compatible naming for Compose
1. **Implemented RandomVerticalFlip** - Flips height dimension only
...
1. Preserve shape and channel structure
...
1. **Assume standard channel counts**:
...
1. **Fixed Channel Count**: Hardcoded to 3 channels (RGB). Could be parametrized.

1. **Nearest-Neighbor Rotation**: Uses simple sampling. Could implement bilinear for better quality.

1. **No Batch Operations**: Transforms operate on single images. Batch support would require shape metadata.
...
1. **Bilinear Interpolation**: For RandomRotation quality improvement
   - Worth doing if rotation artifacts become problematic

1. **In-Place Operations**: Optimize memory usage for large images
   - Profile to determine if worthwhile

1. **Batch Support**: Process multiple images simultaneously
