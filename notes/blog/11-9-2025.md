...
1. **Use MCP servers** - Offload repetitive operations to tool servers instead of LLM reasoning
1. **Target simple tasks first** - Save complex reasoning for when I really need it
...
1. **Implement MCP servers** - Offload repetitive tasks to reduce token usage
1. **Tune agent conciseness** - Reduce over-documentation and over-testing
1. **Get into actual Mojo coding** - Foundation is done, time for implementation
1. **Track token usage per task** - Measure before/after optimization impact
...
1. **Efficiency matters at scale** - 90k tokens per issue doesn't scale to 1,655 issues
1. **Good agents can still be wasteful** - Quality output ≠ efficient output
1. **Limits force better design** - Would I have noticed double planning without hitting the limit? Probably not.
