...
2. sign - sign function (-1, 0, 1)
3. exp - exponential
4. log - natural logarithm
5. sqrt - square root
6. sin - sine
7. cos - cosine
8. tanh - hyperbolic tangent (duplicate from activation)
9. ceil - ceiling
10. floor - floor
11. round - round to nearest
12. trunc - truncate
13. logical_not - logical not
...
2. logical_or - element-wise OR
3. logical_xor - element-wise XOR
...
2. log_backward - log gradient
3. sqrt_backward - sqrt gradient
4. abs_backward - abs gradient
5. clip_backward - clip gradient
...
2. subtract - subtraction with broadcasting
3. multiply - multiplication with broadcasting
4. divide - division with broadcasting
5. floor_divide - floor division
6. modulo - modulo
7. power - power
8. add_backward - addition gradient
9. subtract_backward - subtraction gradient
10. multiply_backward - multiplication gradient
11. divide_backward - division gradient
...
2. prelu (alpha tensor)
3. elu (alpha parameter)
4. gelu (approximate boolean)
...
2. **Elementwise backward passes** (gradient correctness critical)
3. **Elementwise binary operations** (logical ops)
4. **Arithmetic operations** (complex due to broadcasting)
5. **Remaining activations** (parametric functions, lower priority)
...
2. Takes `output` (forward result) - sqrt (for efficiency)
...
2. Validate with test suite after each module
3. Measure performance improvements (eliminate conversion overhead)
4. Document pattern for future contributors
5. Consider creating specialized broadcast dispatcher if needed
