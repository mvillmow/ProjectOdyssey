...
1. `test_normalize_custom_range()` - Normalization to custom [min, max]
1. `test_normalize_already_normalized()` - Input already in range
1. `test_normalize_single_value()` - Single-element tensor
1. `test_normalize_negative_values()` - Negative input values
1. `test_normalize_zero_range()` - All values are the same
1. `test_normalize_batched()` - Batched input handling
1. `test_normalize_inverse()` - Inverse denormalization
...
1. `test_standardize_custom_params()` - Custom mean/std parameters
1. `test_standardize_already_standardized()` - Input already standardized
1. `test_standardize_single_value()` - Single-element tensor
1. `test_standardize_zero_std()` - Constant values (std=0)
1. `test_standardize_batched()` - Batched input handling
1. `test_standardize_inverse()` - Inverse destandardization
1. `test_standardize_computed_stats()` - Compute mean/std from data
...
1. `test_to_float32_from_float64()` - Convert float64 to float32
1. `test_to_int32_from_float()` - Convert float to int32 (truncation)
1. `test_to_int32_rounding()` - Test rounding behavior
1. `test_type_conversion_batched()` - Batched type conversion
1. `test_type_conversion_preserves_values()` - Lossless conversions
...
1. `test_reshape_flatten()` - Flatten to 1D
1. `test_reshape_add_dimension()` - Add batch dimension
1. `test_reshape_remove_dimension()` - Squeeze dimensions
1. `test_reshape_invalid_shape()` - Error on incompatible shapes
1. `test_reshape_batched()` - Reshape within batches
...
1. `test_sequential_two_transforms()` - Two transforms in sequence
1. `test_sequential_multiple_transforms()` - Multiple transforms
1. `test_sequential_empty()` - Empty transform list
1. `test_sequential_single_transform()` - Single transform in sequence
1. `test_sequential_deterministic()` - Reproducible results
...
1. `test_conditional_predicate_false()` - Transform not applied
1. `test_conditional_predicate_true()` - Transform applied
1. `test_conditional_in_pipeline()` - Conditional within sequential
...
1. `test_standardize_inverse_roundtrip()` - Standardize then destandardize
1. `test_inverse_not_supported()` - Error for non-reversible transforms
1. `test_inverse_preserves_values()` - Values match after roundtrip
1. `test_inverse_batched()` - Inverse works with batches
1. `test_inverse_composition()` - Inverse of composed transforms
...
1. `test_data_augmentation_pipeline()` - Augmentation with transforms
1. `test_multi_type_pipeline()` - Convert types and transform
1. `test_batch_processing_pipeline()` - Process batches efficiently
...
1. Standardize tests (8 tests)
1. Type conversion tests (6 tests)
...
1. Conditional transform tests (4 tests)
...
1. Inverse transform tests (6 tests)
...
1. **BatchTransform Tests** (5 tests)
...
1. **Transform Trait**: All transforms implement the existing `Transform` trait from `shared/data/transforms.mojo`

1. **Lambda Functions**: Used `fn (Float32) -> Float32` signature for lambda transforms

1. **Batch Processing**: BatchTransform operates on `List[Tensor]` rather than implementing Transform trait

1. **Type Conversions**: ToFloat32 and ToInt32 preserve data in Tensor wrapper (all stored as Float32 internally)

### TDD Workflow

1. **Write Test**: Define expected behavior
1. **Run Test**: Verify it fails (red)
1. **Implement**: Write minimal code to pass
1. **Run Test**: Verify it passes (green)
...
1. Refactor for performance and clarity
1. Package for integration (Issue #421)
1. Final cleanup and optimization (Issue #422)
