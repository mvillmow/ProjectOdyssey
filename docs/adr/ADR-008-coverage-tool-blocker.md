...
**CI Integration**: `make test-mojo` runs all discovered tests
...
print("   - CI runs all tests via `make test-mojo` (validation only, no metrics)")
print()
...
2. **Run all tests** - `make test-mojo` validates test execution
...
- All tests still run via `make test-mojo`
...
- `make test-mojo` - CI test execution
