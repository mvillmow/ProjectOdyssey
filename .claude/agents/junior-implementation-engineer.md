...
### Example 1: Generate Test Fixture
**Task**: Create test data generator
...
fn create_test_tensor[
    ...
    Returns tensor with sequential values: [0, 1, 2, ..., size-1]
    """
    var result = Tensor[dtype, size]()
    for i in range(size):
        result[i] = i
...
### Example 2: Apply Code Template
**Task**: Create getter/setter methods

**Implementation**:
```mojo
struct Model:
    var _learning_rate: Float32

    fn get_learning_rate(self) -> Float32:
        """Get learning rate."""
        return self._learning_rate

    fn set_learning_rate(inout self, value: Float32):
        """Set learning rate."""
        self._learning_rate = value
```

### Example 3: Format and Lint
**Task**: Format new code files

```bash
# Format Mojo files
mojo format src/mojo/new_module.mojo

# Run Python formatters
black src/ml_odyssey/new_module.py
isort src/ml_odyssey/new_module.py

# Run linters
ruff check src/ml_odyssey/new_module.py

# Fix auto-fixable issues
ruff check --fix src/ml_odyssey/new_module.py
```
